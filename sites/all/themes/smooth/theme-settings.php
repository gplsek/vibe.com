 ...
 */
function smooth_default_theme_settings() {
  $defaults = array(
	'theme_style'							=> 'dark',
 ...
 */
function smooth_initialize_theme_settings($theme_name) {
  ...
  if (!isset($theme_settings['primary_menu_dropdown']) || $theme_settings['rebuild_registry'] == 1) {
    ...
    $defaults = array_merge(smooth_default_theme_settings(), $info_theme_settings);
...
*   array A form array.
*/
function smooth_settings($saved_settings) {
  global $base_url;
  ...
  $form['tnt_container'] = array(
    '#type' => 'fieldset',
    '#title' => t('Smooth theme settings'),
    '#description' => t('Use these settings to enhance the appearance and functionality of your Smooth theme.'),
    '#collapsible' => TRUE,
    '#collapsed' => FALSE,
  );
  ...
  // Appearance settings
  $form['tnt_container']['general_settings']['theme_appearance_config'] = array(
    '#type' => 'fieldset',
    '#title' => t('Appearance'),
    '#collapsible' => TRUE,
    '#collapsed' => TRUE,
  );
 /*  $form['tnt_container']['general_settings']['theme_appearance_config']['background_image'] = array(
    '#type'          => 'select',
    '#title'         => t('Select a Background image'),
    '#default_value' => $settings['background_image'],
    '#options'       => array(
    					'background-bg0'         => t('Mosaic'),
    					'background-bg1'         => t('Bokeh'),
    					'background-bg2'         => t('Hexagonal'),
    					'background-bg3'         => t('Splash'),
    					'background-bg4'         => t('Pattern'),
    					'background-bg5'         => t('Fabric'),
    					'background-custom1'     => t('Custom Background'),        
    )
  ); */
  $form['tnt_container']['general_settings']['theme_appearance_config']['theme_style'] = array(
    '#type'          => 'select',
    '#title'         => t('Select theme style color'),
    '#default_value' => $settings['theme_style'],
    '#options'       => array(
  	  'style-dark' 			=> t('Dark'),  
      'style-violet' 		=> t('Violet'),
	  'style-azure' 		=> t('Azure'),
	  'style-turquoise' 	=> t('Turquoise'),
	  'style-olive' 		=> t('Olive'),
	  'style-lemon' 		=> t('Lemon'),
	  'style-sepia' 		=> t('Sepia'),
	  'style-carmine' 		=> t('Carmine'),
    ),
  );    
