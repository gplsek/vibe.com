 ...
 */
 ...
 $filename = get_filepaths_via_nid($node->book['bid']);
 if($node->field_img_src[0]['view'] == NULL || $node->field_img_src[0]['view'] == ""){
  $node->field_img_src[0]["filepath"] = $filename[0]['path'];
 }
...
<div id="social_network">
 ...
 <div class="clear-block"></div> 
  ...
  <? if($node->field_book_image) {?>
  <div id="story_image"><img src="/<?php print $node->field_book_image[0]["filepath"]; ?>" border="0"></div>
  <? } else if (!empty($node->field_img_src[0])) { ?>
